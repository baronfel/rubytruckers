...
describe 'ship engines' do
  ...
  it 'should calculate standard engine power correctly' do
    @ship.put_tile!(ShipTile.new(Coordinate.new(6,6), nil,nil,SingleEngine.new(),nil), Coordinate.new(6,6))
  ...
  it 'should calculate mixed engine power correctly' do
    @ship.put_tile!(ShipTile.new(Coordinate.new(6,6), nil,nil,SingleEngine.new(),nil), Coordinate.new(6,6))
    @ship.put_tile!(ShipTile.new(Coordinate.new(6,7), nil,nil,DoubleEngine.new(),nil), Coordinate.new(6,7))
    @ship.min_engine_power.should == 1
